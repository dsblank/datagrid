...
def build_header_row(column_names, width):
    ...
    for name in column_names:
        retval += """<th style="width: %spx; border: 1px solid; border-collapse: collapse; background-color: lightgray; padding-left: 10px;">%s</th>""" % (width, name)
    retval += "</tr>"
    return retval
...
def build_row(DATAGRID, group_by, where, r, row, schema, experiment):
    ...
    for c, (column_name, value) in enumerate(row.items()):
            ...
            if isinstance(value, dict):
                if value["type"] == "integer-group":
                    results = select_category(
                        DATAGRID, group_by, where=None, column_name=column_name, column_value=value["columnValue"],
                        where_description=None, computed_columns=None, where_expr=where)
                    if results["type"] == "category":
                        ...
                        image_data = generate_chart_image(results["type"], [trace], IMAGE_SIZE, IMAGE_SIZE)
                        data = f'data:image/png;base64,{base64.b64encode(image_data).decode()}'
                        value = """<img src="%s" style="max-height: %spx; width: 90%%"></img>""" %  (data, max_height)
        ...
        retval += """<td style="border: 1px solid; border-collapse: collapse; text-align: center; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; height: %spx;">%s</td>""" % (max_height, value)
...
def render_image_dialog(BASEURL, group_by, value, schema, experiment):
    if group_by:
        ...
        labels = columns[0].pills("Labels:", sorted(value["assetData"]["labels"]), selection_mode="multi", default=value["assetData"]["labels"])

        asset_data = experiment.get_asset(value["assetData"]["asset_id"], return_type="binary")
        image = generate_image(asset_data)
        draw_annotations_on_image(image, value["assetData"]["annotations"], image.size[0], image.size[1], includes=labels)
